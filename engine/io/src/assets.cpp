...
bool TextureAsset::is_valid() const {
 ...
 if((size_t)width*height*channels != data.size()) return false;
 return true;
}
...
std::optional<MaterialAsset> AssetLoader::load_material(const std::string& path){
 ...
 try{
  ...
  if(fs::file_size(p)>MAX_JSON_BYTES) return std::nullopt;
  std::ifstream f(p);
...
std::optional<TextureAsset> AssetLoader::load_texture(const std::string& path){
 ...
 try{
  fs::path p(path);
  if(!fs::exists(p)) return std::nullopt;
  if(fs::file_size(p)>TextureAsset::MAX_BYTES) return std::nullopt;
...
std::optional<geometry::MeshData> AssetLoader::load_model(const std::string& path){
 ...
 try{
  fs::path p(path);
  if(!fs::exists(p)) return std::nullopt;
  if(fs::file_size(p)>MAX_MODEL_BYTES) return std::nullopt;
  std::ifstream f(p);
  if(!f) return std::nullopt;
